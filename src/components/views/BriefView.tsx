...
const BriefView: React.FC<Props> = ({
  ...
  // Auto-hide research controls once results arrive
  useEffect(() => {
    if (activeSession.researchResults) {
      setShowResearchControls(false);
    }
      ...
      <div className="max-w-3xl mx-auto w-full space-y-4">
        ...
        >
          ...
          <div className="flex items-end gap-2 relative">
            ...
            {/* Web search toggle */}
            <button
                onClick={() => onUpdateSession({ webSearchEnabled: !activeSession.webSearchEnabled })}
                title={activeSession.webSearchEnabled ? 'Web search on — click to turn off' : 'Turn on web search'}
                className={`p-2 rounded-full shrink-0 mb-1 transition-all duration-200 flex items-center justify-center relative z-20 ${
        ...
        {/* Footer — Research */}
        <div
          className="flex flex-col items-center gap-3 mt-6 animate-in fade-in fill-mode-backwards"
                        ...
                        <div className="relative animate-in fade-in slide-in-from-bottom-2 duration-500 ease-out">
                            <div className="flex items-start gap-3 px-4 py-3 rounded-xl bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 shadow-md max-w-[260px]">
                                <div className="flex-1 min-w-0">
                                    <p className="text-xs font-medium text-zinc-800 dark:text-zinc-100 leading-snug">Always know what to write next.</p>
                                    <p className="text-xs text-zinc-500 dark:text-zinc-400 leading-snug mt-0.5">Get fresh trending topics in your niche, automatically.</p>
                                </div>
                                <button
                                    onClick={() => {
                    ...
                    {showResearchControls && researchProfiles.length > 0 ? (
                        <div className="animate-in fade-in slide-in-from-bottom-2 duration-300 ease-out flex items-center gap-2 px-3 py-2 rounded-full bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 shadow-sm">
                            ...
                            <button
                                ...
                                disabled={activeSession.isResearching}
                                className="ml-1 text-[11px] font-semibold bg-zinc-900 dark:bg-zinc-100 text-white dark:text-zinc-900 px-3 py-1 rounded-full transition-colors hover:bg-zinc-700 dark:hover:bg-zinc-300 disabled:opacity-50 flex items-center gap-1.5 shrink-0"
                            >
                                {activeSession.isResearching ? <Loader2 size={10} className="animate-spin" /> : null}
                                Search
                            </button>
                        </div>
                    ) : (

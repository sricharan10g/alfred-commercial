...
const Sidebar: React.FC<Props> = ({
  ...
  // contentCollapsed: controls which layout (icon-only vs full) is rendered
  const [contentCollapsed, setContentCollapsed] = useState(false);
  // showIcons: gates the collapsed-specific icon elements — cleared immediately on expand
  // so icons vanish before the sidebar starts growing
  const [showIcons, setShowIcons] = useState(false);
  // textVisible: drives opacity fade-in of expanded text after sidebar has grown
  const [textVisible, setTextVisible] = useState(true);
  const collapseTimerRef = useRef<ReturnType<typeof setTimeout> | null>(null);
  const collapseTimerRef2 = useRef<ReturnType<typeof setTimeout> | null>(null);
  ...
  const toggleCollapse = () => {
    ...
    if (collapseTimerRef2.current) clearTimeout(collapseTimerRef2.current);

    if (isCollapsed) {
      // EXPANDING:
      // 1. Immediately hide collapsed icons — they vanish before width starts growing
      setShowIcons(false);
      // 2. Pre-set text to invisible so it fades in (not pops in)
      setTextVisible(false);
      // 3. Kick off width animation
      setIsCollapsed(false);
      // 4. After sidebar has grown (~220ms), switch to expanded layout
      collapseTimerRef.current = setTimeout(() => {
        setContentCollapsed(false);
        // 5. Short pause then fade text in
        collapseTimerRef2.current = setTimeout(() => setTextVisible(true), 40);
      }, 220);
    } else {
      // COLLAPSING:
      // 1. Immediately fade out text
      setTextVisible(false);
      // 2. Switch layout & shrink width right away
      setContentCollapsed(true);
      setShowIcons(false);
      setIsCollapsed(true);
      // 3. After sidebar has finished shrinking, reveal icons
      collapseTimerRef.current = setTimeout(() => {
        setShowIcons(true);
        setTextVisible(true); // reset for next expand
      }, 260);
    }
  };
  ...
  useEffect(() => {
    const handleMouseMove = (e: MouseEvent) => {
      ...
      if (isCollapsed) {
        ...
        setIsCollapsed(false);
        setContentCollapsed(false);
        setTimeout(() => setTextVisible(true), 260);
      }
    };
  ...
  // Helper: text fade class — applied to all expanded-view text so it fades in after sidebar grows
  const textFade = `transition-opacity duration-300 ease-in-out ${textVisible ? 'opacity-100' : 'opacity-0'}`;
    ...
    <>
        ...
        >
          {(!contentCollapsed || mobile) && (
            <h1 className={`text-lg font-semibold text-zinc-900 dark:text-white flex items-center gap-2 tracking-tight truncate transition-colors duration-300 ease-in-out ${!mobile ? textFade : ''}`}>
                <Command size={20} className="text-zinc-900 dark:text-white shrink-0 transition-colors duration-300 ease-in-out" />
                Alfred
            </h1>
          )}
          {/* Collapsed expand button — only visible when icons are explicitly shown */}
          {showIcons && !mobile && (
          ...
          {!contentCollapsed && !mobile && (
             <button onClick={toggleCollapse} className={`text-zinc-400 hover:text-zinc-900 dark:text-zinc-600 dark:hover:text-white transition-colors ${textFade}`}>
        ...
        <div className={`shrink-0 transition-all duration-300 ease-in-out overflow-hidden ${showNewBrief ? 'max-h-20 opacity-100 p-4' : 'max-h-0 opacity-0 p-0'}`}>
             <button
                ...
                className={`w-full flex items-center justify-center gap-2 bg-black dark:bg-white hover:bg-zinc-800 dark:hover:bg-zinc-200 text-white dark:text-black rounded-lg font-semibold transition-all mb-4 ${showIcons && !mobile ? 'p-3' : 'px-4 py-2.5 text-sm'}`}
                title="New Brief"
            >
                <Plus size={(showIcons && !mobile) ? 20 : 16} />
                {(!contentCollapsed || mobile) && (
                  <span className={!mobile ? textFade : ''}>New Brief</span>
                )}
            </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar min-h-0 px-2">
            {(!contentCollapsed || mobile) && (
                <p className={`px-3 text-[10px] font-bold ${mobile ? 'text-zinc-500 dark:text-zinc-400' : 'text-zinc-400 dark:text-zinc-500'} uppercase tracking-wider mb-2 flex items-center gap-2 transition-colors duration-300 ease-in-out ${!mobile ? textFade : ''}`}>
                    History
                </p>
            )}
                ...
                {sortedSessions.map(session => (
                    <div
                        ...
                        className={`group w-full rounded-md transition-colors flex items-center cursor-pointer ${
                            (showIcons && !mobile) ? 'justify-center p-2' : 'px-3 py-2 gap-2 text-sm'
                        } ${
                        ...
                        title={(showIcons && !mobile) ? session.name : undefined}
                    >
                        {(showIcons && !mobile) ? (
                            ...
                            <>
                                ...
                                <button
                                    ...
                                    className={`shrink-0 w-2 h-2 rounded-full transition-all duration-200 ${!mobile ? textFade : ''} ${
                                        session.pinned
                                ...
                                />
                                <span className={`truncate flex-1 ${!mobile ? textFade : ''}`}>{session.name}</span>
                                <button
                                    onClick={(e) => onDeleteSession(session.id, e)}
                                    className={`opacity-0 group-hover:opacity-100 p-1 hover:text-red-500 dark:hover:text-red-400 transition-all rounded-md ${!mobile ? textFade : ''}`}
        ...
        >
           <button
             ...
             className={`w-full flex items-center gap-2 py-2 ${mobile ? 'text-zinc-700 dark:text-zinc-300' : 'text-zinc-500'} hover:text-black dark:hover:text-white font-medium transition-colors rounded-md hover:bg-zinc-100 dark:hover:bg-zinc-900 ${(showIcons && !mobile) ? 'justify-center px-0' : 'px-2 text-sm'}`}
             title="Guardrails"
           >
             <ShieldCheck size={20} />
             {(!contentCollapsed || mobile) && <span className={!mobile ? textFade : ''}>Boundaries</span>}
           </button>
           <button
           ...
           >
             <Settings size={20} />
             {(!contentCollapsed || mobile) && <span className={!mobile ? textFade : ''}>Settings</span>}
           </button>
           <button
             onClick={() => mobile ? handleMobileAction(onLogout) : onLogout()}
             className={`w-full flex items-center gap-2 py-2 ${mobile ? 'text-zinc-700 dark:text-zinc-300' : 'text-zinc-500'} hover:text-red-600 dark:hover:text-red-400 font-medium transition-colors rounded-md hover:bg-red-50 dark:hover:bg-red-950/20 ${(showIcons && !mobile) ? 'justify-center px-0' : 'px-2 text-sm'}`}
             title="Sign Out"
           >
             <LogOut size={20} />
             {(!contentCollapsed || mobile) && <span className={!mobile ? textFade : ''}>Sign Out</span>}
           </button>
           {(!contentCollapsed || mobile) && (
             <div className={`flex items-center gap-2 px-2 pt-2 flex-wrap ${!mobile ? textFade : ''}`}>
